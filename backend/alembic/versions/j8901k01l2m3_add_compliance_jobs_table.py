...
def upgrade() -> None:
    compliancejobstatus = sa.Enum('QUEUED', 'RUNNING', 'COMPLETED', 'FAILED', 'CANCELLED', name='compliancejobstatus')
    compliancejobstatus.create(op.get_bind(), checkfirst=True)
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('QUEUED', 'RUNNING', 'COMPLETED', 'FAILED', 'CANCELLED', name='compliancejobstatus'), nullable=False, server_default='QUEUED'),
